...
use crate::extensions::{PublicKeyExt, SignatureExposed};
...
common_hash.update(b"SigEd25519 no Ed25519 collisions");
common_hash.update(&[1]); // Ed25519ph
common_hash.update(&[ctx.len() as u8]);
common_hash.update(ctx);

// Compute H(dom || R || A || H(M)) for each (signature, public_key, message) triplet
let hrams: Vec<Scalar> = (0..messages.len())
    .map(|i| {
        let mut h: Sha512 = common_hash.clone();
        h.update(sigs[i].R.as_bytes());
        h.update(public_keys[i].as_bytes());
        h.update(Sha512::digest(&messages[i]).as_slice());
        Scalar::from_hash(h)
    })
    .collect();
    ...
    let As = public_keys
        .iter()
        .map(|pk| Some(pk.to_decompressed_point()));
    ...
    .ok_or_else(|| ErrorCode::BATCH_VERIFICATION_FAILURE.value())
    .unwrap();
    ...
    T: CryptoRng + RngCore,
{
    let sigs = batch_sigs
        .take_signatures()
        .iter()
        .map(|x| Signature::from_bytes(&x).expect("not decoding sigs").into())
        .collect::<Vec<SignatureExposed>>();
    if sigs.is_empty() {
    ...
    let pks = batch_sigs
        .take_public_keys()
        .iter()
        .map(|x| VerifyingKey::from_bytes(&x).unwrap())
    ...
    verify_batch_unwrapped(
        batch_sigs.messages.as_slice(),
        sigs.as_slice(),
        pks.as_slice(),
        Some(context),
        &mut csprng,
...
mod tests {
    ...
    fn batch_verify_validates_multiple_correct_signatures() {
        ...
        let mut keypairs: Vec<SigningKey> = Vec::new();
        ...
        let public_keys: Vec<VerifyingKey> = keypairs.iter().map(|key| key.public).collect();

        let result = verify_batch_unwrapped(
            &messages,
            &signatures.as_slice(),
            &public_keys,
            Some(context),
